...
export default async function handler(
  ...
  if (req.method === 'POST') {
    ...
    if (!nome || !turma) {
      return res
        .status(400)
        .json({ message: 'Nome e Turma são campos obrigatórios.' });
    }
    ...
    if (error) {
      if (error.code === '23505') {
        return res
          .status(409)
          .json({ message: 'Já existe um aluno com este código (RA).' });
      }
  ...
  if (req.method === 'PUT') {
    ...
    if (typeof id !== 'string') {
      return res
        .status(400)
        .json({ message: 'ID do aluno é obrigatório e deve ser válido.' });
    }

    // Impede alteração direta do saldo
    const { saldo_atual, ...updateData } = req.body;

    if (Object.keys(updateData).length === 0) {
      return res
        .status(400)
        .json({ message: 'Nenhum dado enviado para atualização.' });
    }
    ...
    if (error) {
      if (error.code === '23505') {
        return res
          .status(409)
          .json({ message: 'Já existe outro aluno com este código (RA).' });
      }
